...
- ✅ 正交投影 · 最小二乘的几何本质(`demos/orthogonal-projection/`)
- ⏳ 微分 · 导数
- (后续持续增加)
...
7. **文档里的字符画(ASCII art)对齐部分只用纯 ASCII 字符**(`/ | \ - + * . < > ^ =` 等);
   中文、希腊字母、°、·、全角制表符(╱│─┆●┴▶←)等"宽度不定"字符只能放在
   **行尾标注区**(其后不再有需要对齐的字符)或图外独立行——不同平台的等宽
   字体对这类字符按半角/全角处理不一,行中混排必然错位。需要多列中文对照时
   用 markdown 表格,不要用字符画硬对齐。
