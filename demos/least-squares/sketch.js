...
// 最小绝对偏差(LAD / L1)最优直线:目标 min Σ|eᵢ| 没有统一的闭式公式(本质是
// 线性规划),但有个漂亮性质——总能找到一条最优线穿过其中两个数据点(存在性,
// 不是说每条最优线都必须如此)。于是枚举所有点对(n≤15,最多 105 对)确定候选
// (k, b),取 SAE 最小者即精确解。x 相同的点对对应竖线(斜率无穷),跳过。
// 全部点 x 相同时 k、b 无法确定,返回 null(面板显示"—")。
